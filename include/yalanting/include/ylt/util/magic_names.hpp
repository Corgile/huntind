...
namespace refvalue {namespace detail {
#if __has_include(<span>)
...
#error "Unsupported platform."
    ...
    static constexpr auto value = [] {
      ...
      return meta_string{
      std::span<const char, final_size>{
...
#error "Unsupported compiler."
    ...
    if constexpr (rpos != std::string_view::npos) {
      ...
      if constexpr (right == std::string_view::npos) return str;
      return str.substr(0, right);
    } else {
      constexpr size_t left = result.find("l ") + 2;
      constexpr size_t right = result.find('(');
      if constexpr (left == std::string_view::npos) return result;
      if constexpr (right == std::string_view::npos) return result;
