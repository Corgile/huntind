...
namespace coro_rpc {namespace protocol {
  ...
  public:
    ...
    static std::optional<supported_serialize_protocols> get_serialize_protocol(
      ...
      if (req_header.serialize_type == 0) AS_LIKELY {
    ...
    static async_simple::coro::Lazy<std::error_code> read_head(
      ...
      auto [ec, _] = co_await coro_io::async_read(
        socket, asio::buffer(&req_head, sizeof(req_header)));
      if (ec) AS_UNLIKELY {
        co_return std::move(ec);
      }
      if (req_head.magic != magic_number || req_head.version > VERSION_NUMBER) AS_UNLIKELY {
    ...
    static std::string prepare_response(std::string& rpc_result,
      ...
      if (attachment_len > UINT32_MAX) AS_UNLIKELY {
      ...
      } else if (rpc_result.size() > UINT32_MAX) AS_UNLIKELY {
      ...
      if (rpc_err_code != coro_rpc::errc{}) AS_UNLIKELY {
        rpc_result.clear();
  ...
  using context = context_base<return_msg_type, protocol::coro_rpc_protocol>;
