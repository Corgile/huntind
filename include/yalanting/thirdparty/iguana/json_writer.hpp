...
template<typename Stream, typename T, std::enable_if_t<map_container_v<T>, int>>
IGUANA_INLINE void render_json_value(Stream& ss, const T& o) {
  ss.push_back('{');
...
template<typename Stream, typename T, std::enable_if_t<sequence_container_v<T>, int>>
IGUANA_INLINE void render_json_value(Stream& ss, const T& v) {
  ss.push_back('[');
...
constexpr auto write_json_key = [](auto& s, auto i, auto& t) IGUANA__INLINE_LAMBDA {
  s.push_back('"');
