...
namespace hd::type {
    ...
    private:
        ...
        bool processIPv4Packet(byte_t const *ip4RawBytes) {
            ...
            func<IPPROTO_TCP, tcphdr>("TCP", _ipProtocol, _tcpOrUdp);
            func<IPPROTO_UDP, udphdr>("UDP", _ipProtocol, _tcpOrUdp);
            return true;
        }

        template<uint8_t AssumedIpProtocol, typename HeaderType>
        void func(const char *suffix, const uint8_t actual_ip_protocal, byte_t const *_tcpOrUdp) {
            ...
            m5Tuple.append(sport).append("_")
                    .append(dport).append("_")
                    .append(suffix);
            pktCode = IPPROTO_UDP;
