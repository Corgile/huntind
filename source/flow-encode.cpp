...
torch::Tensor
transform::FlowToTensor(hd_flow const& flow, torch::TensorOptions const& option, torch::Device& device, int width) {
  const int window_count = flow.count - flow.count % width;
  torch::Tensor flow_tensor = torch::empty({window_count, 136}, option);
  std::vector<std::future<void>> futures;
  futures.reserve(window_count);
  for (int _idx = 0; _idx < window_count; ++_idx) {
    futures.emplace_back(std::async(std::launch::async, [&, index = _idx] {
      flow_tensor[index] = PacketToTensor(flow._packet_list[index], flow.protocol, device);
    }));
  }
...
transform::BuildSlideWindowConcurrently(flow_vector const& flow_list, int width, torch::Device& device) {
  ...
  const int batch_count = hd::global::opt.num_cpus;
  const int data_size = static_cast<int>(flow_list.size());
  const int batch_size = static_cast<int>(flow_list.size() + batch_count - 1) / batch_count;
  ...
  for (int i = 0; i < data_size; i += batch_size) {
    futures.emplace_back(std::async(std::launch::async, [&, start = i, end = std::min(i + batch_size, data_size)] {
      ...
      for (int j = start; j < end; ++j) {
        torch::Tensor flow_tensor = FlowToTensor(flow_list[j], window_option, device, width);
...
transform::BuildSlideWindow(flow_vector const& flow_list, int width, torch::Device& device) {
  const auto index_option = torch::TensorOptions().dtype(torch::kI32).device(device);
  const auto window_option = torch::TensorOptions().dtype(torch::kF32).device(device);
  ...
  long offset = 0;
  std::ranges::for_each(flow_list, [&](hd_flow const& flow) {
...
torch::Tensor
transform::MergeFlow(torch::Tensor const& encoded_flows, torch::Tensor const& index_arr, torch::Device& device) {
  ...
  for (int i = 0; i < temp.size(0); ++i) {
    auto merged_flow = encoded_flows.slice(0, ptr[i][0], ptr[i][1]);
...
BatchEncode(torch::jit::Module* model, const torch::Tensor& data, int64_t batch_size) {
  ...
  std::vector<torch::Tensor> GPU_results;
