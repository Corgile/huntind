...
void ProducerEventCb::event_cb(RdKafka::Event& event) {
  ...
  case RdKafka::Event::EVENT_ERROR:ELOG_ERROR << RED("错误: ") << event.str();
    break;
  case RdKafka::Event::EVENT_STATS:ELOG_TRACE << BLUE("EVENT_STATS: ") << event.str();
    break;
  case RdKafka::Event::EVENT_LOG:ELOG_TRACE << BLUE("EVENT_LOG: ") << event.fac();
    break;
  case RdKafka::Event::EVENT_THROTTLE:ELOG_TRACE << RED("EVENT_THROTTLE: ") << event.broker_name();
    break;
  }
}
